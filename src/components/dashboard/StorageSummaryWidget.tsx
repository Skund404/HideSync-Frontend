...
const StorageSummaryWidget: React.FC = () => {
  ...
  // Track if we need to display location details in a future enhancement
  ...
  // Function that would use storageLocations to display additional details
  ...
  return (
    <div className='bg-white rounded-lg shadow-sm border border-stone-200 p-6'>
      <div className='flex justify-between items-center mb-4'>
        ...
        <div className='flex space-x-2'>
          <button
            ...
            className='text-sm text-stone-600 hover:text-stone-800 mr-2'
          >
      ...
      {/* Conditionally show location details */}
      {showLocationDetails &&
        storageLocations &&
        storageLocations.length > 0 && (
          <div className='mb-4'>
            <h4 className='text-sm font-medium text-stone-700 mb-2'>
              All Storage Locations ({storageLocations.length})
            </h4>
            <div className='text-xs text-stone-500'>
              {storageLocations.map((loc) => (
                <div key={loc.id} className='py-1 border-b border-stone-100'>
                  {loc.name} - {loc.type}
                </div>
              ))}
            </div>
